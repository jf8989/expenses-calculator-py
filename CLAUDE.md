...
Claude owns `CLAUDE.md`/`.claude/**`; load `.claude/local/operator-profile.md`. Ignore peers; explicitly use `improve` for cross-runtime changes.

For read-only credential state, say `The non-secret <KEY> row is in credentials.md`; no `I`/`we` action verbs.
