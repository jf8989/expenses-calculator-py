...
import { Debt, ParticipantSummary, CurrencyDebt, MultiCurrencySummaries } from "@/lib/calculations";
...
interface SettleUpProps {
    ...
    multiDebts: CurrencyDebt[];
    mainCurrency?: string;
}

export function SettleUp({ multiSummaries, multiDebts, mainCurrency = "USD" }: SettleUpProps) {
    const { t } = useLanguage();
    const currencies = Object.keys(multiSummaries);

    // Check if any payer mode transactions exist
    const hasPayers = useMemo(() => {
        return Object.values(multiSummaries).some(summaries =>
            Object.values(summaries).some(s => s.totalPaid > 0)
        );
    }, [multiSummaries]);

    // Group debts by currency
    const debtsByCurrency = useMemo(() => {
        const grouped: Record<string, CurrencyDebt[]> = {};
        multiDebts.forEach(d => {
            if (!grouped[d.currency]) grouped[d.currency] = [];
            grouped[d.currency].push(d);
        });
        return grouped;
    }, [multiDebts]);

    const fmt = (amount: number, currency: string) => formatCurrency(Math.abs(amount), currency);
                    ...
                    <CardContent className="space-y-4">
                        {currencies.map((currency) => {
                            ...
                            const sorted = Object.values(summaries)
                                .filter(s => Math.abs(s.balance) > 0.01 || s.totalPaid > 0.01 || s.fairShare > 0.01)
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex gap-4 mt-1.5 ml-6 text-xs text-muted-foreground">
                                                        <span>{t.settle.paid}: <strong className="text-foreground">{fmt(s.totalPaid, currency)}</strong></span>
                                                        <span>{t.settle.fairShare}: <strong className="text-foreground">{fmt(s.fairShare, currency)}</strong></span>
                                                    </div>
                                                </motion.div>
                                            );
                                        })}
                                                ...
                                                {debts.map((debt, idx) => (
                                                    ...
                                                    >
                                                        <div className="flex flex-col">
                                                            ...
                                                            <span className="text-2xl font-bold gradient-text">{fmt(debt.amount, currency)}</span>
                                                        </div>
