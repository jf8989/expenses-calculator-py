...
export const firestoreService = {
  async getUserData(userId: string): Promise<UserData | null> {
    ...
    return (userDoc.data() as UserData) || null;
  },
  ...
  async saveSession(userId: string, session: Omit<Session, "id">) {
    ...
    const res = await db.collection("users").doc(userId).collection("sessions").add(sessionData);
    await this.updateLastUpdated(userId);
    return res.id;
  },

  async updateSession(userId: string, sessionId: string, session: Partial<Session>) {
    const sessionData = {
      ...session,
      lastUpdatedAt: admin.firestore.FieldValue.serverTimestamp(),
    };
    await db.collection("users").doc(userId).collection("sessions").doc(sessionId).update(sessionData);
    await this.updateLastUpdated(userId);
  },
